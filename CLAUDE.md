...
- The list of rooms lives in `assets/rooms.js` (number, folder slug, name, deadpan blurb). The lobby grid and the "rooms" menu inside every room are both built from it, so a new room is one line there with the next number. The lobby lists rooms strictly in number order. `featured: true` only adds a small "recommended by management" note to that room's row.
...
- Room numbers are permanent and there are no gaps. (Until the site settles, Cole may still reorder: `assets/rooms.js` sorts itself by `n`, so a move is just swapping numbers. A few numbers are baked into content: 015 is the mirror in the backrooms and on its closed sign, 023 is the secret room and its hints, 999 is about, and the basement puts the unnumbered door after 010.) Retiring a room means keeping its entry in `assets/rooms.js` with `defunct: '<deadpan reason>'` (e.g. mirror: "closed. it was weird. even for here."). Closed rooms show struck through with a red "closed" stamp and aren't clickable in the lobby and the rooms menu, and prev/next skip them. Replace the room's folder with a small closed-sign page (`mirror/index.html` is the template) so old links land somewhere funny instead of a 404, and give its basement door `defunct: '<reason>'` so it's boarded up. Our failures can be funny.
- A room with `hidden: true` in `assets/rooms.js` is a secret: it's left out of every list until the visitor unlocks it. 023 "nothing" is the secret room permanently; new rooms start at 024. It unlocks with the Konami code on the lobby (arrow keys, or swipes and two taps on touch; `localStorage` `cizole.nothing`), which also turns the basement's unnumbered "not yet." door into its door. The hints are deliberate and stay: the lobby shows an empty row where 023 would be (hover reveals "there is no 023", clicking it types most of the code into the tagline), the about page's FAQ denies it, the 404 department denies it, and the browser console hints at it.
...
- Navigation stays in the same tab. rooms.js turns every in-site link into a short "walk": a veil drops with the destination's number and name, then the page changes, and the next page fades up (the fade-in is in `site.css`). A room that navigates after its own animation calls `window.CIZOLE_GO(href)` (or just `location.assign` if it has its own fade, like the basement). Never `window.open` a room; the only exception is the other window room, where extra windows are the point. Links to outside sites (about page) may use `target="_blank"`.
- Rooms get an edge affordance for free: on pointer devices, hovering the left or right edge of the screen slides in the previous/next room. A room entered through a basement door gets "← basement" as its top-left link instead of the lobby (rooms.js checks the referrer). Keep the middle of the screen's left and right edges free of room UI.
